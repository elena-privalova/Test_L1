...
describe('Test TargetComponent', () => {
    ...
    it('render', () => {
        ...
        <ReduxProvider reduxStore={store}><DetailCard {...data} /></ReduxProvider>
        </BrowserRouter>)
        ...
        data.tags.forEach((tag) => expect(screen.getByText(tag.value)).toBeInTheDocument())

        expect(screen.getByText(getFormattedDate(data.createdAt))).toBeInTheDocument();
    })
    ...
    it('render', () => {
        ...
        expect(screen.getAllByText((_, element) => element!.tagName.toLowerCase() === 'div')[2]).toHaveClass('detail-card__header--auth');
    ...
    it('render', () => {
        ...
        expect(screen.getAllByRole('radio').filter((item, index) => item.id === `:r${index+1}:`).length).toEqual(5);
    })
    ...
    it('render', () => {
        ...
        const fourRate = screen.getAllByRole('radio').find(item => item.id === `:r${4}:`);
    ...
    it('render', () => {
        ...
        const authEmail = screen.getAllByText((_, element) => element!.tagName.toLowerCase() === 'span')[1];
